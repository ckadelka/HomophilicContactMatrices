...
by further Boolean attributes. For each Boolean attribute, its prevalence across the population is known.
For attribute1, the population may also exhibit positive homophily, h in [0,1], such that
there are more contacts among people with the same attribute value. 
...
class ContactMatrix2:
    def __init__(self, C_old, N_expanded, multipliers_attribute2, SAME_ROW_SUM=True, RECIPROCITY=True, HOMOPHILY=True, CORRECT_AGE_AGE_CONTACTS=True, norm=2):
        self.C_old = np.array(C_old)
        self.N_old = np.sum(N_expanded,(1,2))#np.array(N_old)
        self.N = N_expanded
        ...
        self.prevalence_new_attributes_given_old_index = np.array([N_expanded[i]/np.sum(N_expanded[i]) for i in self.indices_old])
        self.N_times_multipler = np.array([np.multiply(N_expanded[i],self.multipliers_attribute2) for i in self.indices_old])
        ...
        self.desired_rowsum_multiplier_without_multiplier = np.array([np.sum(N_expanded[i])/np.sum(self.N_times_multipler[i]) for i in self.indices_old])
...
if __name__ == '__main__':
    ...
    contact_matrix = m.solve(homophily=0.5)
    plot_4x4_contact_matrix(contact_matrix.reshape((4,4)))
    ...
    m = ContactMatrix2(C,N,N_new,multipliers_attribute2)
    CC = m.solve(1)
    ...
    # #Real U.S. data
    names_attributes = [['0-15','16-64','65-74','75+'],['POC','WA'],['LC','HC']]
    empirical_contact_matrix = np.array([[7.48,5.05,0.18,0.04],[1.96,12.12,0.21,0.04],[0.93,3.75,1.14,0.15],[0.91,2.70,0.49,0.40]])#np.array(pd.read_csv('interactionmatrix/age_interactions.csv'))[:,1:]
    census_data = np.array([ 60570126, 213610414,  31483433,  22574830])
    prevalence_WorA_ethnicity = np.array([0.5481808309264538, 0.652413561634687, 0.7960244996154009, 0.8233809069658553])
    prevalence_high_contact = np.array([[0,0],[0.45,0.15],[0,0],[0,0]])
    census_data_split = np.zeros((len(census_data),2,2))
    for i in range(len(census_data)):
        for v in range(2):
            for w in range(2):
                census_data_split[i,v,w] = census_data[i]*(prevalence_WorA_ethnicity[i] if v==1 else 1-prevalence_WorA_ethnicity[i])*(prevalence_high_contact[i,v] if w==1 else 1-prevalence_high_contact[i,v])
    multipliers_attribute2 = [1,3]
    for homophily_attribute1 in [0,0.8,1]:
        m = ContactMatrix2(empirical_contact_matrix,census_data,census_data_split,multipliers_attribute2)
        CC = m.solve(homophily = homophily_attribute1)
        (C_out,N_out,indices_out) = m.delete_empty_populations(CC)
        plot_contact_matrix_specific(C_out,N_out,indices_out,names_attributes,'JTB_study_homotphily%i.pdf' % int(100*homophily_attribute1))
    ...
    C = np.array([[7.48,5.05,0.18,0.04],[1.96,12.12,0.21,0.04],[0.93,3.75,1.14,0.15],[0.91,2.70,0.49,0.40]])#np.array(pd.read_csv('interactionmatrix/age_interactions.csv'))[:,1:]
